...
// TYPES
// ─────────────────────────────────────────────────────────────────────────────
type Category = "Country" | "City" | "Animal" | "Food" | "Celebrity" | "Brand" | "Object";
type ValidationEntry = { valid: boolean; reason: string };
type ValidationMap = Record<Category, ValidationEntry>;
type AnswersMap = Partial<Record<Category, string>>;

type QueueEntry = { id: string; name: string; joinedAt: number };
...
type Room = {
  ...
  answers: Record<string, AnswersMap>; // keyed by player name
  validation: Record<string, ValidationMap>; // keyed by player name
};

// ─────────────────────────────────────────────────────────────────────────────
// STORAGE HELPERS (localStorage-based)
// ─────────────────────────────────────────────────────────────────────────────
...
const CATS: Category[] = ["Country", "City", "Animal", "Food", "Celebrity", "Brand", "Object"];
...
// ─────────────────────────────────────────────────────────────────────────────
// MATCHMAKING (localStorage "queue" — works only on the SAME device/browser)
// ─────────────────────────────────────────────────────────────────────────────
async function joinQueue(entry: QueueEntry): Promise<{ role: "joiner"; room: Room } | { role: "waiter" }> {
  let q = (await dbGet<QueueEntry[]>(QUEUE_KEY)) || [];
  const now = Date.now();

  // keep only fresh entries (90s) and not me
  q = q.filter((e) => now - e.joinedAt < 90_000 && e.id !== entry.id);
...
async function pollForMyRoom(myId: string): Promise<string | null> {
  return await dbGet<string>(`ag-myroom:${myId}`);
}

async function removeFromQueue(myId: string): Promise<void> {
  const q = (await dbGet<QueueEntry[]>(QUEUE_KEY)) || [];
  await dbSet(
    QUEUE_KEY,
    q.filter((e) => e.id !== myId)
  );
}

// ─────────────────────────────────────────────────────────────────────────────
// "AI" VALIDATION
// IMPORTANT: Browser-to-Anthropic will fail (CORS + secret key). Use backend.
// This function defaults to a local fallback validator.
// ─────────────────────────────────────────────────────────────────────────────
async function aiValidate(answers: AnswersMap, letter: string): Promise<ValidationMap> {
  // If you have a backend endpoint that calls Anthropic safely, put it here:
  // const API_URL = "/api/validate"; // your server
  // and return await fetch(API_URL, { ... })
  // For now: fallback validator to keep the game working.

  const fb = {} as ValidationMap;
  CATS.forEach((c) => {
    const v = (answers[c] || "").trim();
    const ok = v.length >= 2 && v.toLowerCase().startsWith(letter.toLowerCase());
    fb[c] = { valid: ok, reason: ok ? "Starts with the letter" : "Invalid or empty" };
  });
  return fb;
}

function totalScore(v: ValidationMap | null): number {
  if (!v) return 0;
...
type GameBoardProps = {
  letter: string;
  answers: AnswersMap;
  onChangeAnswer: (cat: Category, value: string) => void;
  submitted: boolean;
  validating: boolean;
  timeLeft: number;
  oppName: string | null;
...
function GameBoard({
  ...
  return (
    <>
      ...
      <div className="tnum" style={{ color: tcol }}>
      ...
      {validating ? (
        <div className="vwrap">
          ...
          <div className="aibadge">
            <div className="aidot" /> AI is judging your answers…
          </div>
...
export default function App() {
  ...
  const [oppAnswers, setOppAnswers] = useState<AnswersMap>({});
  const [oppVal, setOppVal] = useState<ValidationMap | null>(null);
  const [error, setError] = useState<string>("");
  const [queueStatus, setQueueStatus] = useState<"searching" | "found">("searching");

  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const pollAnsRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const answersR = useRef<AnswersMap>({});
  const letterR = useRef<string>("B");
  const roomR = useRef<string>("");
  const nameR = useRef<string>("");
  const myIdR = useRef<string>("");
  const submittedR = useRef<boolean>(false);

  // keep refs in sync without causing re-renders that break focus
  useEffect(() => {
    answersR.current = answers;
  }, [answers]);
  useEffect(() => {
    letterR.current = letter;
  }, [letter]);
  useEffect(() => {
    nameR.current = playerName;
  }, [playerName]);
  useEffect(() => {
    submittedR.current = submitted;
  }, [submitted]);
  ...
  useEffect(() => {
    return () => {
      ...
      if (pollAnsRef.current) clearInterval(pollAnsRef.current);
    };
  }, []);
  ...
  // ── Submit & validate ──────────────────────────────────────────────────────
  async function doSubmit(l: string, roomId: string | null) {
    ...
    if (roomId) {
      try {
        const room = await dbGet<Room>(`ag-room:${roomId}`);
        if (room) {
  ...
  const handleChangeAnswer = useCallback((cat: Category, val: string) => {
    setAnswers((prev) => ({ ...prev, [cat]: val }));
  }, []);

  // ── Launch online game ─────────────────────────────────────────────────────
  function launchOnlineGame(room: Room, myName: string) {
  ...
  async function findMatch() {
    ...
    const entry: QueueEntry = { id: myId, name: playerName, joinedAt: Date.now() };
        ...
        pollRef.current = setInterval(async () => {
          try {
            ...
            if (roomId) {
              ...
              if (room) {
                setQueueStatus("found");
                setTimeout(() => launchOnlineGame(room, playerName), 800);
              }
            }
          } catch {
            // ignore
              ...
              {queueStatus === "found" ? (
                ...
                <>
                  ...
                  <div className="mstatus">
                    Looking for an opponent for <strong style={{ color: "var(--txt)" }}>{playerName}</strong>…
                  </div>
      ...
      <div className="G">
        ...
        <div className="S">
          <div className="sscrn">
            ...
            <div className="smax">
              out of {CATS.length * 10} · letter {letter}
            </div>
            <div className="aibadge" style={{ fontSize: 12 }}>
              <div className="aidot" /> validated
            </div>
            ...
            <div style={{ display: "flex", gap: 10, width: "100%" }}>
              <button className="btn btn-p" style={{ flex: 1 }} onClick={startSolo}>
                Play Again
              </button>
              <button className="btn btn-g" style={{ flex: 1 }} onClick={() => setScreen("home")}>
                Home
              </button>
            </div>
          </div>
      ...
      <div className="G">
        ...
        <div className="S">
          <div className="vsscrn">
            ...
            <div className="aibadge" style={{ alignSelf: "center", fontSize: 12 }}>
              <div className="aidot" /> validated
            </div>

            <div className="cmp">
              {CATS.map((cat) => {
                const ma = answers[cat] || "";
                const oa = oppAnswers[cat] || "";
                const mv = validation?.[cat];
                const ov = oppVal?.[cat];
                const mp = mv?.valid ? 10 : 0;
                const op2 = ov?.valid ? 10 : 0;
                ...
                return (
                  <div key={cat} className="cmpr">
                    ...
                    <div
                      className={`cmpc ${op2 > mp ? "cw" : op2 < mp ? "cl" : ""}`}
